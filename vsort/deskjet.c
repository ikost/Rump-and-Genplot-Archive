...
#define DEFAULT_FILENAME	"deskjet.out"
...
----------------------------------------- */
		case 0:
			...
			if (! NoCompress) {								/* Need buffers			*/
				...
				if (TransferInfo.Class != DJET && TransferInfo.Class != DJET_PLUS) {
...
--------------------------------------------------------------------------- */
			...
			IO_fprintf(io, HPSetPlanes, 0);				/* Number of CMY planes	*/
...
/* -- Find first non-zero element (or determine line to be empty) */
				...
				if (NoCompress) {								/* Are we to compress?	*/
					...
					idelta = irll+itiff+1;					/* Never smallest			*/
					if (DeltaBuffer != NULL)				/* Is Delta possible?	*/
