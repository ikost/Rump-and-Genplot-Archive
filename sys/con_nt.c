...
/* ------------------------------- */
EXPORT void *SysMainWindowHwnd=NULL;				/* Set by XGENPLOT/XRUMP */
EXPORT void *SysGraphWindowHwnd=NULL;				/* Set by PMDRIV */
...
/* Handles for Win 32 API */
HANDLE hwndStdin  = INVALID_HANDLE_VALUE;
HANDLE hwndStdout = INVALID_HANDLE_VALUE;
HANDLE hwndStderr = INVALID_HANDLE_VALUE;
int ConIsPipe = TRUE;
...
static void CONTerminate(void) {
	if (! ConIsPipe && ConIsInitialized) {
		SetConsoleScreenBufferSize(hwndStdout, OldConsoleSize);
	}
...
/* Register my termination address on exit */
	atexit(CONTerminate);
	
/* Behavior depends on whether I have a pipe connected mode, or real */
	ConIsPipe = ! isatty(fileno(stdin));

	hwndStdin  = GetStdHandle(STD_INPUT_HANDLE);
	hwndStdout = GetStdHandle(STD_OUTPUT_HANDLE);
	hwndStderr = GetStdHandle(STD_ERROR_HANDLE);
	SetConsoleTitle("Genplot NT Console Version 0.91");
	...
	if (! ConIsPipe) {
		...
		SysResolveDyntName(path, "genplot_.ini", sizeof(path));
		NewSize.X = GetPrivateProfileInt("CONSOLE", "BUFFER_COLUMNS", 80, path);
		NewSize.Y = GetPrivateProfileInt("CONSOLE", "BUFFER_ROWS",    50, path);

		GetConsoleScreenBufferInfo(hwndStdout, &buf);
		OldConsoleSize = buf.dwSize;							/* Save for a restore */
		SetConsoleScreenBufferSize(hwndStdout, NewSize);
	}
...
=========================================================================== */
void CONRestart(void) {
	if (! ConIsInitialized) CONInitialize();
	return;
}
...
int CONwaitchr(int msecs) {
	...
	if (msecs == 0) {								/* Really equivalent to CONgetc() */
		rcode = CONgetc();
	} else if (! ConIsPipe) {					/* real keyboard ... use multiple waits */
		msecs = (msecs+50)/100;					/* Closest number of 100 ms intervals */
		if (msecs < 1) msecs = 1;				/* Always at least a 100 ms wait */
		while (msecs--) {
			if ( (rcode = CONchkchr()) != -1) break;
			MilliSleep(100);
		}
	} else {											/* Check if already waiting */
		int hold;
		hold = SysXtermCharCount;				/* How many characters have been transferred */
		msecs = (msecs+50)/100;					/* Closest number of 100 ms intervals */
		if (msecs < 1) msecs = 1;				/* Always at least a 100 ms wait */
		while (msecs--) {
			if (hold != SysXtermCharCount) {
				rcode = fgetc(stdin);
				break;
			}
			MilliSleep(100);
...
static void my_SIGINT(int sig) {
   ...
   fputs("-> BreakHandler invoked <-\n", stderr);
	...
	/* Tell everyone who requested notification */
	for (i=0; i<MAX_BREAK_NOTIFY; i++) {
		if (BreakNotifyList[i] != NULL) *BreakNotifyList[i] += 1;
	}

	/* And put something into the stack so looks like it was pressed */
