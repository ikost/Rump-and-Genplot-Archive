...
/*  main.c -- initializes code and calls genplot */
...
#include "lexp.h"
#include "tplot.h"
#include "genplot.h"							/* For any gpt_do_ routines */
...
/* ============================================================================
-- Shell program for calling GENPLT with a variable size work buffer.  Handles
-- the numerous complaints for a different size program.
--
...
#define	USAGE_TEXT	"GENPLOT [-Buffer <npoints>] [-Inifile <file>] [-Quiet] [-Debug <n>] [-Help]"
...
/* .. Check for a valid serial number module .. */
	if (! SysCheckLicense("Genplot")) abort();
...
/* .. Process command line options */
	...
	*aptr = '\0';									/* Terminate the string */
	LexInsText(token);
	if ( (aptr=getenv("GENPLOT")) == NULL) aptr = getenv("genplot");
	...
	while (LexGetToken(token, TOKEN_SIZE)) {
		...
		if (LexEqual(aptr, "help", 1) || strcmp(aptr,"?")==0) {
			...
			continue;
		} else if (LexEqual(aptr, "buffer", 1) || LexEqual(aptr, "bufsize", 4)) {
			BufferSize = LexGetInt(BufferSize, "Buffer size: ");
			continue;
		} else if (LexEqual(aptr, "3D", 2)) {
			mode = MODE_3D;
			continue;
		} else if (*token == '-') {
			ERRprintf("\n",
				"ERROR: %s is an invalid option for GENPLOT invocation.\n\n"
				"\n"
				"       " USAGE_TEXT "\n"
				"\n"
	...
	if (LexGetFile(token, TOKEN_SIZE)) {			/* Treat rest */
		if (access(token, R_OK) != 0) {				/* If not, just leave on stack */
			...
			switch (CheckFileType(token, ext)) {
				case F_SPECTRUM:
					sprintf(tmpstr, "read '%s' plot", fullname);
					break;
				case F_HCOPY:
...
/* ... Print welcome screens */
	...
	TTYputs("GENPLOT Shell for NT, OS/2 & Posix  [Rev. 1.02 12/28/98 (c) CGS]\n");
...
/* ... Start up the initialization file if configured */
	if (*inifile == '\0') {
		if ( (aptr=getenv("GENPLOT.INI")) == NULL) aptr = getenv("genplot.ini");
		if (aptr == NULL) aptr = "genplot.ini";
		strcpy(inifile, aptr);
	}
	SysResolveDyntName(inifile, inifile, sizeof(inifile));	/* Resolve ini file */
	...
	if (mode == MODE_2D) {
		if (! GVAlloc2DCurve("$PLOT", 0, BufferSize)) panic;	/* NOT GVF_USER! */
	} else {
		if (! GVAlloc3DCurve("$PLOT", 0, BufferSize)) panic;	/* NOT GVF_USER! */
	}
	GVSetValue("$plot:npt", "0");						/* No points initially */
...
/*		TTYprintf("MSG: Re-initializing signal handler for signal %d\n", rcode); */
	...
	while (TRUE) {
		...
		ERRputs("ERROR: QUIT must be used to exit GENPLOT, not RETURN\n");
	}
	return(rcode & 0xFF);
}
...
"              º A Data Analysis and Graphical Plotting Program º\n"
"              º          for Scientists and Engineers          º\n"
"              ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼\n"
"\n"
"      GGGGGGG    EEEEEEE  NNN     NN   PPPPPPPP   LL         OOOOOOO   TTTTTTTT\n"
"    GG     GG   EE       N NN    NN   PP     PP  LL        OO     OO     TT\n"
"   GG          EE       NN NN   NN   PP     PP  LL        OO     OO     TT\n"
"  GG  GGGGG   EEEEE    NN  NN  NN   PPPPPPPP   LL        OO     OO     TT\n"
" GG     GG   EE       NN   NN NN   PP         LL        OO     OO     TT\n"
"GG     GG   EE       NN    NNNN   PP         LL        OO     OO     TT\n"
"GGGGGGG    EEEEEEE  NN     NNN   PP         LLLLLLLL   OOOOOOO      TT\n"
"\n"
"                          Computer Graphic Service\n"
"                              130 Oakwood Lane\n"
"                           Ithaca, New York 14850\n"
"                         Phone/FAX: (607) 273-4927\n"
"              email/WEB: support@genplot.com www.genplot.com\n"
#else
"              |================================================|\n"
...
"             -----------------------------------------------------\n"
#endif
"                          <press any key to continue>", SysSerialNumber);
	fflush(stdout);
	if (quiet == 0) sleep(1);
	return;
}
...
static FILETYPE CheckFileType(char *path, char *ext) {
	...
	if ( (icnt=EA_QueryAsciiList(path, ".TYPE", &mylist)) > 0) {
		for (i=0; i<icnt; i++) {
			if (stricmp(mylist[i], "Genplot Data File") == 0) {
				ftype = F_SPECTRUM; break;
			} else if (stricmp(mylist[i], "Genplot Macro") == 0) {
				ftype = F_MACRO; break;
...
/* If the type is still not determined, use the extension or assume macro */
	if (ftype == F_NONE) {					/* Keep trying to identify */
		if (stricmp(ext, ".dat") == 0 || stricmp(ext, ".data") == 0) {
			ftype = F_SPECTRUM;
